...
export const AuthMethods = {
  ...
  async createPassword(event) {
    ...
    const securityAnswer = document
      .getElementById("setupSecurityAnswer")
      .value.trim();
    ...
    if (!securityQuestion) {
      this.showToast("Please select a security question", "error");
      return;
    }
    if (!securityAnswer) {
      this.showToast(
    ...
    if (userIndex === -1) {
      this.showToast("Staff ID not found. Contact admin.", "error");
      return;
    }
  ...
  async verifyStaffForReset(event) {
    ...
    if (!user) {
      this.showToast("Staff ID not found", "error");
      return;
    }

    if (!user.securityQuestion) {
      this.showToast(
        "Security question not set up. Please contact your administrator.",
        "error",
      );
      return;
    }
  ...
  async resetPassword(event) {
    ...
    if (answer !== this.tempResetStaff.securityAnswer.toLowerCase()) {
