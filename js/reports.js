...
export const ReportMethods = {
  async loadReportFilters() {
    ...
    document.getElementById("reportDept").innerHTML =
    ...
    document.getElementById("reportToDate").value = today
      .toISOString()
      .split("T")[0];
    document.getElementById("reportFromDate").value = lastWeek
      .toISOString()
      .split("T")[0];
  },
  ...
  async generateReport() {
    ...
    let filtered = attendance
      .filter((a) => {
        ...
        if (department && (!staff || staff.department !== department))
          return false;
        return true;
      })
      .sort(
  ...
  async exportReport() {
    ...
    const csv = [
      "Date,Staff ID,Name,Department,Role,Check-in Time,Status",
      ...filtered.map((a) => {
        const staff = users.find((u) => u.staffId === a.staffId);
  ...
  async loadStaffProgress() {
    ...
    const onTimeDays = myAttendance.filter(
      (a) => a.status === "ON_TIME",
    ).length;
    ...
    document.getElementById("staffProgressTable").innerHTML =
      myAttendance
        .map(
          (a) =>
        ...
        .join("") ||
      '<tr><td colspan="3" class="text-center" style="padding: 24px; color: var(--text-secondary);">No attendance records</td></tr>';
  },
